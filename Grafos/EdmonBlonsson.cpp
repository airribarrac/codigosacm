...
// RECORDAR SETEAR LA VARIABLE GLOBAL  V Y EL VECTOR DE VISITADOS ED O SINO NO FUNCIONA!!
#include <bits/stdc++.h>
using namespace std;
const int M=500;
...
int main(){
	...
	for(int i = 0; i < n; i++){
		...
		//		cout << s <<endl;
			...
			jueces[i].push_back(s);
		}
	}
	//printf("distintos = %d\n",distintos);	
	vector <vector <bool> > adj(distintos,vector <bool>(distintos,true));
	for(int i = 0; i < n; i++){
		for(int j =0 ; j < m; j++){
			for(int k = j+1; k < m; k++){
				adj[M[jueces[i][j]]][M[jueces[i][k]]] = false;
				adj[M[jueces[i][k]]][M[jueces[i][j]]] = false;
			}
		}
	}
	V = distintos;
	...
	int final =edmonds();
	if(final >= abs(m-distintos))puts("S");
	else puts("N");
  return 0;
}